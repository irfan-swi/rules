...
def visualize_and_summarize_changes(rule, title):
    ...
    # fig1,fig2,fig3,fig4 = create_plot(merged_df)
        ...
        st.warning('Not enough congresses or Rule/Title not selected')
...
def to_title_case(option_list):
    return [option.title() for option in option_list]
...
with tabs[0]:
    ...
    with col2:
        rule_options = ['All'] + sorted(data['Rule'].unique().tolist())
        rule_selection = st.selectbox("Rule Selection", to_title_case(rule_options), key='rule')
        filter_new_rule = st.checkbox("New Rule Only", value=False)
# Title selection box
    with col3:
        if rule_selection.lower() != 'all':
            title_options = ['All'] + sorted(data[data['Rule'].str.lower() == st.session_state.rule.lower()]['Title'].unique().tolist())
        else:
            title_options = ['All'] + sorted(data['Title'].unique().tolist())
        title_selection = st.selectbox("Rule Title", to_title_case(title_options), key='title', index=0)
    ...
    # with col3:
    #     if rule_selection.lower() != 'all':
    #         title_options = ['All'] + sorted(data[data['Rule'].str.lower() == rule_selection.lower()]['Title'].unique().tolist())
    #     else:
    #          title_options = ['All'] + sorted(data['Title'].unique().tolist())   
    #     title_selection = st.selectbox("Rule Title", to_title_case(title_options), key='title', index=0)
    #     filter_new_title = st.checkbox("New Title Only", value=False)
    ...
    with col5:
        speaker_selection = st.selectbox('Speaker', to_title_case(['All'] + sorted(data['Speaker'].unique().tolist())), key = 'speaker')
    with col6:
        search_query = st.text_input("Search Rule, Title, and Text", key = 'search')
        filter_changed_text = st.checkbox("Changed Text Only", value=False)
    with col_reset:
        st.write('')
        st.write('')
        st.button("Reset", on_click = reset)

    #convert selection to lower for processing
    rule_selection = rule_selection.lower()
    title_selection = title_selection.lower()   
    speaker_selection = speaker_selection.lower()      
    ...
    final_data.Title = final_data.Title.str.title()
    final_data.Rule = final_data.Rule.str.title()
    final_data.Speaker = final_data.Speaker.str.title()
    final_data['Prior Rule'] = final_data['Prior Rule'].str.title()
    final_data['Prior Title'] = final_data['Prior Title'].str.title()
